...
fetch(`http://localhost:8600/user/tutor?id=${teacherId}`)
.then((res)=>res.json())
.then((data)=>{
    render(data.msg)
    console.log(data)
...
function render(element){
    ...
    <div id="detailsdiv">
       ...
       <button id="delete">ACCEPT</button>
    </div>
</div>`
main.innerHTML=card
...
dele.addEventListener("click",()=>{
    fetch(`http://localhost:8600/user/tutor/${teacherId}`,{
       method:"PATCH" ,
       headers:{
        "Content-Type":"application/json"
       },
       body:JSON.stringify({appointed:true})
    })
    .then((res)=>res.json())
    .then((data)=>{
        console.log(data)
